...
export default function SingleAvailableUnitPage() {
  ...
  useEffect(() => {
    ...
    if (savedResort) {
      try {
        const parsed = JSON.parse(savedResort);
        setCurrentResort(parsed);
        if (parsed.vacationType) {
    ...
    const queryVacationType = searchParams.get("vacationType");
    if (queryVacationType) {
      setVacationType(queryVacationType);
    }
  }, [searchParams]);
