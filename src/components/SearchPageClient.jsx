...
export default function SearchPageClient() {
  ...
  const searchData = useMemo(() => {
    ...
    return performSearch(searchTerm, allResortData);
  }, [allResortData, searchTerm]);

  const resortsCount = searchData.length;
        ...
        {searchData.length > 0 ? (
          ...
          <div className="mt-10 rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
            <p className="text-lg text-slate-600">
              Start a search from the header to find resorts by name, location,
              or ID.
            </p>
          </div>
        )}
